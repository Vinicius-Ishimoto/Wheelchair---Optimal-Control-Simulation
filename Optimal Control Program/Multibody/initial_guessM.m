function xg = initial_guessM(setup)

xg = [];
% M = legslb(setup.mesh_number+1);
% M = 0.5*(M+1);
for w=1:setup.assist.nP
    state = [setup.initial_guess.phase(w).position,setup.initial_guess.phase(w).velocity];
    for i=1:setup.phase(w).assist.n
        xg = [xg;interp1_lg(setup.initial_guess.phase(w).time,state(:,i),sum(setup.mesh.phase(w).colpoints))];
    end
