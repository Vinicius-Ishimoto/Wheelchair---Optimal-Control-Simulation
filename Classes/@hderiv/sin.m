...
for j=1:size(in,2)
    for i=1:size(in,1)
        ...
        jH.ddV = sparse(1:numel(in(i,j).value),1:numel(in(i,j).value),-sin(in(i,j).value))*jH.ddV;
        in(i,j).ddV = sparse(1:numel(in(i,j).value),1:numel(in(i,j).value),cos(in(i,j).value))*in(i,j).ddV;
